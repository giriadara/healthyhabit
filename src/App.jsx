  ...
  const handlePay = async () => {
    ...
    const res = await fetch("/api/create-order", {
      ...
      body: JSON.stringify({
        ...
        notes: { name, variant: selectedProduct.name, qty }
      }),
    });
          ...
          {PRODUCTS.map((p) => (
            <article key={p.sku} className="rounded-3xl bg-white shadow hover:shadow-lg transition p-4 flex flex-col">
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <button onClick={() => { setVariant(p.sku); setOpen(true); }} className="px-4 py-2 rounded-2xl bg-emerald-600 text-white hover:bg-emerald-700">Book</button>
              </div>
            </article>
          ))}
        </div>
        ...
        <div className="mt-6 grid md:grid-cols-3 gap-6">
          {[...PRODUCTS].map((p, i) => (
            <div key={p.sku} className={`rounded-3xl p-6 shadow bg-white ${i===1?"ring-2 ring-emerald-500":""}`}>
              ...
              <p className="mt-4 text-3xl font-extrabold">₹{p.price}<span className="text-sm font-medium text-slate-500"> / bowl</span></p>
              ...
              <button onClick={() => { setVariant(p.sku); setOpen(true); }} className="mt-5 w-full px-4 py-3 rounded-2xl bg-emerald-600 text-white hover:bg-emerald-700">Book {p.name}</button>
            </div>
          ))}
        </div>
      </section>
          ...
          <div className="mt-6 rounded-3xl bg-white shadow p-6 grid md:grid-cols-2 gap-5">
            <div className="space-y-4">
              <label className="block">
                ...
                <input value={name} onChange={(e)=>setName(e.target.value)} className="mt-1 w-full border rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500" placeholder="e.g., Priya"/>
              </label>

              <label className="block">
                <span className="text-sm font-medium">Choose a bowl</span>
                <select value={variant} onChange={(e)=>setVariant(e.target.value)} className="mt-1 w-full border rounded-xl px-3 py-2 focus:ring-2 focus:ring-emerald-500">
            ...
            <div className="space-y-4">
              <label className="block">
                ...
                <textarea value={address} onChange={(e)=>setAddress(e.target.value)} rows={4} className="mt-1 w-full border rounded-xl px-3 py-2" placeholder="Flat / Street / Landmark"/>
              </label>

              <label className="block">
                <span className="text-sm font-medium">Notes</span>
                <textarea value={notes} onChange={(e)=>setNotes(e.target.value)} rows={3} className="mt-1 w-full border rounded-xl px-3 py-2" placeholder="Allergies, no pineapple, extra pomegranate, etc."/>
              </label>
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                ...
                <a href={`mailto:orders@healthyhabit.example?subject=Fruit%20Bowl%20Booking&body=${encodeURIComponent(`Name: ${name}\nVariant: ${selectedProduct.name}\nQty: ${qty}\nDate: ${date} ${time}\nAddress: ${address}\nNotes: ${notes}`)}`} className="text-center px-4 py-3 rounded-2xl border border-emerald-600 text-emerald-700 hover:bg-emerald-50">Email</a>
                <button onClick={handlePay} className="px-4 py-3 rounded-2xl bg-black text-white">Pay Online</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <footer className="max-w-6xl mx-auto px-4 py-10">
        <div className="flex flex-col md:flex-row items-start justify-between gap-6">
          ...
          <div className="text-sm text-slate-600">
            <p>Order / WhatsApp: <a className="text-emerald-700 font-semibold" href="https://wa.me/9000925013" target="_blank" rel="noreferrer">9000925013</a></p>
